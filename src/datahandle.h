...
class DataHandle
{
    public:
        ...
        void setFadParameters(const double* parameters, const double* eParameters,const double& fad_cov_0_1,const int which);
        ...
        double getIntegral(const int which,const int* integralRange) const;
        std::pair<double,double> getSignal(const int which,const int* signalRange=NULL,const int* backgroundRange=NULL);

        inline void setMainWindowData(MainWindowData& data){_mainWindowData=data;}
        const double* computeCalibration(const int* signalRange=NULL,const int* backgroundRange=NULL,Function function=Linear);
