...
This class is tightly bound to \hyperlink{classQCPAxis}{Q\-C\-P\-Axis}. Every axis owns a grid instance internally and uses it to draw the grid. Normally, you don't need to interact with the \hyperlink{classQCPGrid}{Q\-C\-P\-Grid} instance, because \hyperlink{classQCPAxis}{Q\-C\-P\-Axis} reproduces the grid interface in its own interface.

The axis and grid drawing was split into two classes to allow them to be placed on different layers (both \hyperlink{classQCPAxis}{Q\-C\-P\-Axis} and \hyperlink{classQCPGrid}{Q\-C\-P\-Grid} inherit from \hyperlink{classQCPLayerable}{Q\-C\-P\-Layerable}). So it is possible to have the grid at the background and the axes in the foreground, and any plottables/items in between. This described situation is the default setup, see \hyperlink{classQCPLayer}{Q\-C\-P\-Layer} documentation. 
...
You shouldn't instantiate grids on their own, since every \hyperlink{classQCPAxis}{Q\-C\-P\-Axis} brings its own \hyperlink{classQCPGrid}{Q\-C\-P\-Grid} internally 
...
Zero lines are lines at coordinate 0 which may be drawn with a different pen than other grid lines. To disable zero lines and just draw normal grid lines at zero, set {\itshape pen} to Qt\-::\-No\-Pen. 
